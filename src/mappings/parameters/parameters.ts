...
function registerParameterMapping(
    parameterType: string,
    mapping: (scope: Construct, id: string, props: Omit<CloudFormationParameter, "Type">) => TerraformVariable,
) {
...
function simpleTypeMapping(
    from: string,
    to: string,
    mapDefault: (value: string) => unknown,
    validations: TerraformVariableValidationConfig[] = [],
) {
...
function basePropertyValidations(
    valueRef: string,
    props: Omit<CloudFormationParameter, "Type">,
...
export function registerParameterMappings() {
    ...
    simpleTypeMapping("AWS::EC2::KeyPair::KeyName", VariableType.STRING, (value) => value, [{
        ...
        errorMessage:
            `value must be a valid EC2 key pair name containing only alphanumeric characters, hyphens, and underscores`,
    }]);
